...
class OptunaSweeperImpl(Sweeper):
    ...
    def sweep(self, arguments: List[str]) -> None:
        ...
        while n_trials_to_go > 0:
            ...
            if n_trials_to_go % 10 == 0:
                print(f"Saving Trial nr = {n_trials_to_go}")
                save_progress(directions=directions, sweep_dir=self.config.hydra.sweep.dir,
                              best_trial=study.best_trial, trials=study.trials)
        ...
        if len(directions) < 2:
            ...
            save_progress(directions=directions, sweep_dir=self.config.hydra.sweep.dir,
                          best_trial=study.best_trials, trials=study.trials)


def save_progress(directions, sweep_dir, best_trial, trials):
    results_to_serialize: Dict[str, Any]
    all_res = []
    if len(directions) < 2:
        results_to_serialize = {
            "target": best_trial.value,
            "params": best_trial.params,
        }
        log.info(f"Best parameters: {best_trial.params}")
        log.info(f"Best value: {best_trial.value}")
        for trial in trials:
            all_res.append({"target": trial.value, "params": trial.params})
    else:
        pareto_front = [
            {"params": t.params, "values": t.values} for t in best_trial
        ]
        results_to_serialize = {
            "name": "optuna",
            "solutions": pareto_front,
        }
        log.info(f"Number of Pareto solutions: {len(best_trial)}")
        for trial in trials:
            all_res.append({"target": trial.value, "params": trial.params})
        for t in best_trial:
    ...
    OmegaConf.save(
        ...
        f"{sweep_dir}/optimization_log.yaml",
    )
    OmegaConf.save(
        OmegaConf.create(results_to_serialize),
        f"{sweep_dir}/optimization_best.yaml",
    )
